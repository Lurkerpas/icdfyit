...
- **OpenYaml** / **SaveAsYaml** (delegates to `YamlPersistence`, ICD-FUN-154, ICD-FUN-155).
...
### 4.6 YAML Persistence (`YamlPersistence`)

Serialises and deserialises a `DataModel` to/from YAML using the **YamlDotNet** library (ICD-FUN-150, ICD-FUN-151).

**Conventions**

| Convention | Detail |
|---|---|
| Naming | `UnderscoredNamingConvention` maps PascalCase C# properties to `snake_case` YAML keys. |
| Null omission | `DefaultValuesHandling.OmitNull` — absent optional fields are equivalent to null. |
| Entity keys | Entity names are used as dictionary keys; GUIDs do not appear in YAML (ICD-IF-460). |
| References | All cross-entity references are encoded as **names** (ICD-IF-441 to ICD-IF-445). A missing name resolves to `null` without throwing. |
| String-only fields | Dual-representation fields (`NumericIdStr`, `BitSizeStr`, hex values, etc.) are written verbatim as strings, preserving hex notation (ICD-IF-450). |
| Type discriminator | `DataType` entries carry a `type` key with values: `sint`, `uint`, `float`, `bool`, `bitstring`, `enum`, `struct`, `array`. |

**Multi-file composition** (ICD-IF-420, ICD-IF-421)

Each YAML file may declare an `includes` list at its root. On import, `LoadWithIncludes` recursively resolves all includes relative to the including file's directory and merges them into a single `YamlDocument`. Merge rules:

- Includes are processed first (depth-first); the **root file wins** on key collisions.
- Circular includes are detected via a per-call-stack `processing` set and raise `InvalidDataException`.
- De-duplication: a `visited` set prevents the same canonical file from being processed more than once.
- The highest `version` value seen across all files is propagated to the merged document.

**Format versioning**: The root `version` key is checked after all includes are merged. If the effective version exceeds `CurrentVersion`, import is rejected with `InvalidDataException`.

**Import mapping** uses two passes for `DataType`s and `Parameter`s to support mutual and self-referential associations (struct fields → other types; `ValidityParameter` → sibling parameter). Fresh GUIDs are generated by model constructors — YAML never carries GUIDs.

**Classes**

| Class | Location | Responsibility |
|---|---|---|
| `YamlDocument` | `Persistence/Yaml/YamlDtos.cs` | Root DTO: version, includes, entity dictionaries |
| `YamlDataTypeDto` | same | Flat union DTO for all data type kinds |
| `YamlParameterDto` | same | Parameter DTO with name-based refs |
| `YamlPacketTypeDto` | same | Packet type DTO; HeaderIdValues keyed by `HeaderTypeId.Name` |
| `YamlPersistence` | `Persistence/YamlPersistence.cs` | `Export(DataModel, path)` and `Import(path) → DataModel` |

### 4.7 Error Handling & Logging (`LogManager`)
...
- **Menu bar**: New / Open / Save / Save As / **Open from YAML…** / **Save as YAML…**, Validate Model, Exit, Options, Windows (Data Types, Parameters, Header Types, Export), Help / About.
- **Open from YAML…** (ICD-FUN-155): prompts the user for a `.yaml` / `.yml` file; the guard for unsaved changes is applied first; the model is imported via `YamlPersistence`.
- **Save as YAML…** (ICD-FUN-154): prompts the user for an output `.yaml` path; syncs all ChangeNotifier collections then calls `YamlPersistence.Export`.
...
│   │   ├── Persistence/           XmlPersistence, YamlPersistence, migration helpers
│   │   │   └── Yaml/              YamlDtos.cs — YamlDocument + per-entity DTOs
│   │   ├── Export/                ExportEngine (uses pythonnet NuGet package)
│   │   └── Infrastructure/        OptionsManager, LogManager, TemplateSetImporter
...
│   └── IcdFyIt.Cli/              (net8.0 console application)
│       └── Program.cs             Verbs: validate, export, yaml-to-xml, xml-to-yaml
└── tests/
    └── IcdFyIt.Core.Tests/       (net8.0 xUnit test project)
        ├── Model/
        ├── Services/
        ├── Infrastructure/
        └── Persistence/           XmlPersistenceTests, YamlPersistenceTests
```
...
| Name-based YAML references | Avoids GUID leakage into human-editable files; missing names resolve to null rather than throwing (ICD-IF-441 to ICD-IF-445). |
| Multi-file YAML composition | Separates concerns across include files; merge rules (includes first, root wins on collision) give predictable override semantics (ICD-IF-420, ICD-IF-421). |
...
| YAML Serialization | YamlDotNet 16.x | MIT |
